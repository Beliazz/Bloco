	  ...
	  m_activeQueue(0)
{
	//Open up access to script.
	{
		...
		m_MetaTable.RegisterObjectDirect( "SetScriptEventManager", (EventManager *)0, &EventManager::SetScriptEventManager );
	...
	IEventData const & inEvent ) const
{
	...
	if(m_pScriptEventManager)
	{
		m_pScriptEventManager->HandleEvent(inEvent);
	}
	...
	EventListenerMap::const_iterator it =
		m_registry.find( inEvent.VGetEventType().getIdent() );
...
bool EventManager::VTick ( unsigned long maxMillis )
{
	...
	// swap active queues, make sure DEBUG_CLIENTBLOCK queue is empty after the
	// swap ...
	...
	while ( m_queues[queueToProcess].size() > 0 )
	{
		...
		EventListenerMap::const_iterator itListeners =
			m_registry.find( eventType.getIdent() );
		...
		if ( itWC != m_registry.end() )
		{
			...
			//g_pApp->m_pDebugView->HandleEvent(event);
		}
		...
		if(m_pScriptEventManager)
		{
			m_pScriptEventManager->HandleEvent(*event);
		}
	...
	EventType const & inType ) const
{
	...
	if ( ( inType.getIdent() == 0 ) &&
		(strcmp(inType.getStr(),kpWildcardEventType) != 0) )
		return false;
...
bool EventManager::AddScriptListener( char const * const pEventName, LuaObject callbackFunction )
{
	...
	const unsigned int eventID = testEventType.getIdent();
	...
	//Now let's rez up a DEBUG_CLIENTBLOCK script listener.
	ScriptEventListenerPtr listener( DEBUG_CLIENTBLOCK ScriptEventListener( callbackFunction ) );
...
bool EventManager::AddScriptActorListener( char const * const pEventName, LuaObject callbackFunction, const int actorID )
{
	...
	//Now let's rez up a DEBUG_CLIENTBLOCK script listener.
	ScriptActorEventListenerPtr listener( DEBUG_CLIENTBLOCK ScriptActorEventListener( callbackFunction, actorID ) );
...
{
	//Create a DEBUG_CLIENTBLOCK script-defined event object.
	const EventType eventType( pEventName );
...
{
	IRegisteredEventPtr metaData( DEBUG_CLIENTBLOCK EventManager::ScriptDefinedEvent( eventType ) );
	AddRegisteredEventType( eventType, metaData );
}

void EventManager::RegisterCodeOnlyEvent( const EventType & eventType )
{
	IRegisteredEventPtr metaData( DEBUG_CLIENTBLOCK CodeOnlyDefinedEvent() );
	AddRegisteredEventType( eventType, metaData );
}
...
bool EventManager::SetScriptEventManager( LuaObject em )
{
	m_pScriptEventManager = shared_ptr<ScriptEventManager>(DEBUG_CLIENTBLOCK ScriptEventManager(em));
...
ScriptEventManager::ScriptEventManager( LuaObject luaObj ):
m_luaObject(luaObj)
{
...
char const * ScriptEventManager::GetName( void )
{
	return "ScriptEventManager";
}

bool ScriptEventManager::HandleEvent( IEventData const & event )
{
	if(!m_luaObject["FireEvent"].IsFunction())
	{
		printf("Error:   ScriptEventManager::HandleEvent()\n");
		return false;
	}

	LuaFunction<bool> function( m_luaObject["FireEvent"] );

	function(m_luaObject,event.VGetEventType().getStr());
}

void ScriptEventManager::Update()
{
	if(!m_luaObject["Update"].IsFunction())
	{
		printf("Error:   ScriptEventManager::Update()\n");
		return;
	}

	LuaFunction<bool> function( m_luaObject["Update"] );
