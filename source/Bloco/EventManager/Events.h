...
struct BLOCO_API EvtData_RunScript : public BaseEventData
{
	static const EventType sk_EventType;
	virtual const EventType & VGetEventType( void ) const
	{
	...
	explicit EvtData_RunScript( string value )
		: m_filename(value)
	{
	}

	explicit EvtData_RunScript( std::istream & in )
	{
		in >> m_filename;
	}

	explicit EvtData_RunScript( LuaObject srcData )
	{
		m_bHasLuaEventData = true;	//Our Lua event data got passed into us!
	}
	...
	virtual IEventDataPtr VCopy() const
	{
		return IEventDataPtr ( DEBUG_CLIENTBLOCK EvtData_RunScript ( m_filename ) );
	}
	virtual ~EvtData_RunScript() {}
	virtual LuaObject VGetLuaEventData(void) const
	{
		assert( ( true == m_bHasLuaEventData ) && "Can't get lua event data because it hasn't been built yet!  Call BulidLuaEventData() first!" );
		return m_LuaEventData;
